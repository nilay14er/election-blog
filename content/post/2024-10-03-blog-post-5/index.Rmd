...
electoral_votes <- read.csv("corrected_ec_1948_2024.csv") %>%
  filter(year == 2016)

us_map <- map_data("state")
electoral_votes$state <- tolower(electoral_votes$state)

map_data <- left_join(us_map, electoral_votes, by = c("region" = "state"))

ggplot(data = map_data) +
  geom_polygon(aes(x = long, y = lat, group = group, fill = as.factor(electors)), color = "white") +
  scale_fill_viridis_d(name = "Electoral Votes") +
  labs(title = "2016 Electoral College Votes by State") +
  coord_fixed(1.3) +
  theme_minimal()
...
ar_model <- Arima(national_data$mean_diff, order = c(1, 0, 0))  
