...
popvote$winner <- as.logical(popvote$winner)  
incumbent_win_data <- popvote %>%   filter(incumbent == TRUE) %>%   summarise(Won = sum(winner, na.rm = TRUE), Lost = sum(!winner, na.rm = TRUE)) 
incumbent_win_data <- data.frame(   Outcome = c("Incumbent Won", "Incumbent Lost"),   Count = c(incumbent_win_data$Won, incumbent_win_data$Lost) )  

ggplot(incumbent_win_data, aes(x = Outcome, y = Count, fill = Outcome)) +   geom_bar(stat = "identity") +   labs(title = "Incumbent Win Rate in Presidential Elections (1948-2020)",        x = "Election Outcome",        y = "Number of Elections") +   scale_fill_manual(values = c("skyblue", "lightcoral")) +   theme_minimal()
...
popvote <- popvote %>%
  mutate(
    party_last = lag(party),
    reelect_party = party == party_last
  )

incumbent_party_win_data <- popvote %>%
  filter(reelect_party == TRUE) %>%
  summarise(
    Won = sum(winner == TRUE, na.rm = TRUE),
    Lost = sum(winner == FALSE, na.rm = TRUE)
  )

incumbent_party_win_data <- data.frame(
  Outcome = c("Incumbent Party Won", "Incumbent Party Lost"),
  Count = c(incumbent_party_win_data$Won, incumbent_party_win_data$Lost)
)

ggplot(incumbent_party_win_data, aes(x = Outcome, y = Count, fill = Outcome)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Incumbent Party Win Rate in Presidential Elections (1948-2020)",
    x = "Election Outcome",
    y = "Number of Elections"
  ) +
  scale_fill_manual(values = c("seagreen3", "indianred2")) +
  theme_minimal()
...
```{r, echo = FALSE, message=FALSE}
 ...
 poll_summary <- state_polls %>%   
   group_by(year, party) %>%   
   summarise(mean_poll_support = mean(poll_support, na.rm = TRUE))  
 ggplot(poll_summary, aes(x = year, y = mean_poll_support, color = party)) +   
   geom_line(size = 1) +   
   labs(title = "Average Polling Support by Party (1968-2024)",      
        x = "Year",        y = "Polling Support (%)") +   scale_color_manual(values = c("blue", "red")) +   theme_minimal()
```
