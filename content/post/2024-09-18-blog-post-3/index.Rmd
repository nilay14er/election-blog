...
president_polls_2024 <- read.csv("president_polls_2024.csv")
econ_data <- read.csv("fred_econ.csv")

biden_approval_polls_2024 <- read.csv("biden_approval_polls_2024.csv")
harris_approval_polls_2024 <- read.csv("harris_approval_polls_2024.csv")
pres_pollaverages_2020 <- read.csv("pres_pollaverages_2020.csv")
pres_pollaverages_2024 <- read.csv("pres_pollaverages_2024.csv")
state_polls_1968_2024 <- read.csv("state_polls_1968-2024.csv")
```

```{r, echo=FALSE, message=FALSE}
# Filter for North Carolina in the state-level polling data for 2024
nc_state_polling <- dplyr::filter(state_polls, state == "North Carolina")
...
nc_state_polling_filtered <- subset(nc_state_polling, format(poll_date, "%Y") %in% c("2020"))

events <- data.frame(
  event = c("DNC 2020", "RNC 2020", "Presidential Debate"),
  date = as.Date(c("2020-08-17", "2020-08-24", "2020-09-29"))
)
...
ggplot(nc_state_polling_filtered, aes(x = poll_date, y = poll_support)) +
  ...
  labs(x = "Date", y = "Polling Support", title = "Polling Averages by Date, 2020") + 
...
econ_data_recent <- dplyr::filter(econ_data, quarter == 2) %>%
  dplyr::select(year, GDP_growth_quarterly, unemployment)

nc_state_polling_econ <- dplyr::left_join(nc_state_polling, econ_data_recent, by = "year")


nc_recent <- dplyr::filter(nc_state_polling_econ, year >= 2000)

ggplot(nc_recent, aes(x = GDP_growth_quarterly, y = poll_support, color = party, size = abs(GDP_growth_quarterly))) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed") +
  labs(x = "Quarterly GDP Growth (%)",
       y = "Polling Support",
       title = "Polling Support vs. Quarterly GDP Growth (2000-2024)") +
  theme_minimal() +
  scale_size_continuous(name = "GDP Growth Magnitude") +
  facet_wrap(~ year)
```
...
library(ggplot2)
nc_state_polling_econ_filtered <- subset(nc_state_polling_econ, format(poll_date, "%Y") >= "2000")

ggplot(nc_state_polling_econ_filtered, aes(x = unemployment, y = poll_support, color = unemployment)) +
  geom_point(size = 2, alpha = 0.7) +  
  geom_smooth(method = "lm", se = FALSE, linetype = "dashed") +  
  scale_color_gradient(low = "green4", high = "red3") +  
  labs(x = "Unemployment Rate (%)", 
       y = "Polling Support", 
       title = "Polling Support vs. Unemployment Rate (2000-2024)") +
  theme_minimal() +  
  facet_wrap(~ year)  
